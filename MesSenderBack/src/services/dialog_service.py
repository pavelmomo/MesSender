...
class DialogService:
    @staticmethod
    async def get_active_user_dialogs(uow: AbstractUOW, user_id: int, limit: int, offset: int) -> List[DialogDTO] | None:
        async with uow:
    ...
    @staticmethod
    async def create_dual_dialog(uow: AbstractUOW, uid: int, remote_uid: int) -> DialogCreateRespDTO | None:
        if uid == remote_uid:
            return None
        async with uow:
